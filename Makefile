...
override CLINIC_AGENT_PASSWORD := $(value CLINIC_AGENT_PASSWORD)
...
db-bootstrap: db-inputs
	@PGPASSWORD="$${POSTGRES_PASSWORD}" $(DOCKER) exec -i -e PGPASSWORD \
		...
		-e CLINIC_SUPER_PASSWORD -e CLINIC_AGENT_PASSWORD \
		"$${POSTGRES_CONTAINER}" \

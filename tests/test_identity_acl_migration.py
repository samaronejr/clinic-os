...
from identity_acl_migration_support import (
    FOUNDATION_TARGETS,
    LOWEST_ROLE,
    MIGRATION_NAME,
    ORG_A,
...
def test_foundation_upgrade_canonicalizes_all_users_and_keeps_lowest_role() -> None:
...
def test_identity_migration_reverses_and_reapplies() -> None:
    try:
        migrate(FOUNDATION_TARGETS)
        with connection.cursor() as cursor:
            cursor.execute(
